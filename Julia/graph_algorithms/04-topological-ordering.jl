struct DirectedGraph
    ...
    topologicalordering::Vector{Int}
...
function explore(node::Int, state::DFSState, graph::DirectedGraph)::Set{Int}
    ...
    while !isempty(stack)
        ...
        if unvisited === nothing
            ...
            push!(graph.topologicalordering, lastnode)
        else
...
function main()
    ...
    result = depthfirstsearch(graph)
    println(join(reverse(graph.topologicalordering), " "))
end

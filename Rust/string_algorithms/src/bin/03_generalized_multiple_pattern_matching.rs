...
impl Trie {
    ...
    fn has_leaf_child(&self, node: i32) -> bool {
        match self.adjacency_list.get(&node) {
            Some(children) => children.iter().any(|(_, label)| *label == '$'),
            None => false,
        }
    }
    ...
    fn match_text_with_patterns(&self, text: &str) -> bool {
        ...
        for chr in text.chars() {
            ...
            if self.has_leaf_child(next_node) {
                return true
...
fn read_patterns(nr_patterns: i32) -> Vec<String> {
    ...
    for _ in 0..nr_patterns {
        let mut pattern = read_line().trim().to_string();
        pattern.push('$');
...
fn generalized_multiple_pattern_matching(text: String, patterns: Vec<String>) -> Vec<usize> {
...
fn main() {
    ...
    let result = generalized_multiple_pattern_matching(text, patterns);

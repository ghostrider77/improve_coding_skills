...
fn calc_topological_ordering(graph: DirectedGraph) -> Vec<i32> {
    let (_, dfs_state) = graph.run_dfs();
    let mut node_ids = (1..=graph.nr_nodes).collect::<Vec<_>>();
    node_ids.sort_by_key(|&id| Reverse(dfs_state.postvisit_numbers[id as usize - 1]));
    node_ids
}
...
fn main() {
    ...
    let result = calc_topological_ordering(graph);
